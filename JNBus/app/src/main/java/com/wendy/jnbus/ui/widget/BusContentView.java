    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        switch (BusViewConstant.Position.valueOf(position)){
            case RIGHT:
                ...
                imageView.layout( getWidth()-3*stationNameWidth/2 , 0 , getWidth()-stationNameWidth/2 ,stationNameWidth);
            ...
            case LEFT:
                ...
                imageView.layout( stationNameWidth , 0 , 2*stationNameWidth,stationNameWidth);
                imageView1.layout(getWidth()/2 , 0 , getWidth()/2+stationNameWidth,stationNameWidth);
                break;
            case TOP_LEFT:
                textView.layout( getWidth()- 3*stationNameWidth/2, getHeight()-stationNameWidth , getWidth()-stationNameWidth/2,getHeight());
                imageView.layout( stationNameWidth/2, getHeight()-2*stationNameWidth- 2* ((int)radius) , 3*stationNameWidth/2, getHeight()- stationNameWidth- 2* ((int)radius));
                imageView1.layout( stationNameWidth/2, getHeight()/2-stationNameWidth , 3*stationNameWidth/2, getHeight()/2);
                break;
            case TOP_RIGHT:
                textView.layout( getWidth()- 3*stationNameWidth/2, getHeight()-stationNameWidth , getWidth()-stationNameWidth/2,getHeight());
                imageView.layout( getWidth()-3*stationNameWidth/2, getHeight()-2*stationNameWidth- 2* ((int)radius) , getWidth()-stationNameWidth/2, getHeight()- stationNameWidth- 2* ((int)radius));
